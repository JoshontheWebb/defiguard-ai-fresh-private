...
async def root():
    ...
    logger.debug("Flushing log file after root redirect")
    for handler in logging.getLogger().handlers:
        handler.flush()
...
STRIPE_PRICE_PRO = "price_1SFoLlEqXlKjClpjoNrA0XCO"
...
async def webhook(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.debug("Flushing log file after webhook error")
        for handler in logging.getLogger().handlers:
            handler.flush()
        return Response(status_code=500, content=f"Webhook processing failed: {str(e)}")
...
async def upload_temp(file: UploadFile = File(...), username: str = Query(None), db: Session = Depends(get_db), request: Request = None):
    ...
    try:
        ...
        logger.debug("Flushing log file after upload temp error")
        for handler in logging.getLogger().handlers:
            handler.flush()
    ...
    logger.debug("Flushing log file after successful upload")
    for handler in logging.getLogger().handlers:
        handler.flush()
...
async def api_audit(username: str, api_key: str, db: Session = Depends(get_db)):
    try:
        ...
        logger.debug("Flushing log file after API audit")
        for handler in logging.getLogger().handlers:
            handler.flush()
        return {"message": "API audit endpoint (Pro tier)"}
    except Exception as e:
        logger.error(f"API audit error for {username}: {str(e)}")
        logger.debug("Flushing log file after API audit error")
        for handler in logging.getLogger().handlers:
            handler.flush()
        raise HTTPException(status_code=500, detail=f"Internal error: {str(e)}")
...
async def upgrade_page():
    try:
        ...
        logger.debug("Flushing log file after upgrade page error")
        for handler in logging.getLogger().handlers:
            handler.flush()
        raise HTTPException(status_code=500, detail=f"Internal error: {str(e)}")
...
async def get_facets(contract_address: str, request: Request, username: str = Query(None), db: Session = Depends(get_db)):
    try:
        ...
        logger.debug("Flushing log file after facet endpoint error")
        for handler in logging.getLogger().handlers:
            handler.flush()
        raise HTTPException(status_code=500, detail=f"Internal error: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            for contract in slither.contracts:
                logger.debug(f"Processing contract: {contract.name} for {effective_username}")
            ...
            logger.error(f"Slither analysis failed for {effective_username}: {str(e)}")
        ...
        if user.has_diamond and file_size > 1024 * 1024:
            ...
            if user:
                ...
                if overage_mb > 0 and user.stripe_subscription_id and user.stripe_subscription_item_id:
                    ...
                    db.commit()
            logger.debug(f"Audit completed for {effective_username} with Diamond tier, aggregated results: risk_score={aggregated['risk_score']}")
                    ...
                    if user:
                        ...
                        if overage_mb > 0 and user.stripe_subscription_id and user.stripe_subscription_item_id:
                            try:
                                ...
                                logger.error(f"Failed to report overage for {effective_username}: {str(e)}")
                            db.commit()
                    logger.debug(f"Audit completed for {effective_username}, results: risk_score={audit_json.get('risk_score', 'N/A')}")
        ...
        logger.debug("Flushing log file after audit processing")
        for handler in logging.getLogger().handlers:
            handler.flush()
